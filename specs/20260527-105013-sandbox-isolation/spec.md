...
- **`gh` PATH-shim escape** (`--allow-gh-cli` only): a `gh` script earlier on `PATH` could in principle run outside the sandbox. **Confirmed 2026-05-27 (R4 spike): prompted and did not escape (exit 127, no out-of-workspace write)** — the env-prefixed invocation isn't auto-approved and the shim never executed.
- **`gh` compound command** (`--allow-gh-cli` only): a chained command such as `gh pr view 1 && cat /etc/hosts`. **Confirmed 2026-05-27 (R4 spike): the whole compound runs outside the sandbox (`gh *` matches the full line), but it is NOT auto-approved — the `&&` prevents matching the read-only rules, so it always prompts.** Accepted residual risk; the permission prompt is the backstop.
